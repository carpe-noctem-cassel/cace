...
public:
	...
	typename ::std::enable_if< ::std::is_class<C>
	{}>::type>
	...
	{
		return
		{	object_ptr, method_stub<C, method_ptr>};
	}
	...
	{
		return
		{	const_cast<C*>(object_ptr), const_method_stub<C, method_ptr>};
	}
	...
	{
		return
		{	&object, method_stub<C, method_ptr>};
	}
	...
	{
		return
		{	const_cast<C*>(&object), const_method_stub<C, method_ptr>};
	}
	...
	void reset()
	{	stub_ptr_ = nullptr; store_.reset();}
...
namespace std
{
	...
	struct hash<::delegate<R(A...)> >
	{
		size_t operator()(::delegate<R(A...)> const& d) const noexcept
		{
			...
			return hash<typename ::delegate<R(A...)>::stub_ptr_type>()(d.stub_ptr_) + 0x9e3779b9 + (seed << 6)
					+ (seed >> 2);
		}
	};
}
