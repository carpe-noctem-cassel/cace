...
int main(int argc, char **argv) {
	...
	floodingNode::nodeID = std::atoi(argv[1]);
	cout << "Node ID is: " << floodingNode::nodeID << endl;
	...
	ros::init(numberOfParams, argv2, (string("flooding_test_node") + argv[2]).c_str());
	 ...
	 */
	floodingNode::chatter_pub = n.advertise<flooding_test::TestMessage>(
			"flooding_test/TestMessage", 10);
	 ...
	 */
	ros::Subscriber sub = n.subscribe("flooding_test/TestMessage", 30,
			floodingNode::messageCallback);

	//flooding_test::TestMessage msg;
	//if (floodingNode::nodeID == std::atoi(argv[1])) {
	//		msg.id = rand();
	//		msg.sender = floodingNode::nodeID;
	//		floodingNode::chatter_pub.publish(msg);
